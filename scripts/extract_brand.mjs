 ...
 *
 *   node scripts/extract_brand.mjs [--deck knowledge/sources/<deck>.pptx] [--domain <event-domain>]
 *
...
if (!deckPath) {
  ...
  if (candidates.length === 1) deckPath = resolve(dir, candidates[0]);
    ...
    console.error(`knowledge/sources holds ${candidates.length} decks — pass --deck to pick one:`);
    for (const c of candidates) console.error(`  --deck knowledge/sources/${c}`);
    process.exit(2);
  }
}
